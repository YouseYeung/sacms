...
$(document).ready(function(){
	...
	function modifySlideshow()
	{
		var slideshow = $("#slideshow");
		if($(window).width() < 1000)
		{
			slideshow.css("height", slideshow.width() * 0.306);
		}
		else
		{
			slideshow.css("height", 291);
		}
	}
	...
	$(window).resize(function() {
		modifySlideshow();
		$("#navmenu ul li:has(ul)").unbind();
		if($(window).width() > 640 - 17)
		{
			$("#navmenu ul li ul li ul").css("display","none");
			setNormalMenu();
		}
		else
		{
			$("#navmenu ul li ul li ul").css("display","block");
			setMobileMenu();
		}
	});
